...
import { searchDepartments } from '../../../utils/search-utils';
...
class DepartmentSingleSelect extends Component {
  ...
  componentDidMount() {
    ...
    const { enableSelectRange } = this.props;
    if (enableSelectRange) {
      this.initRangeDepartments();
    } else {
  ...
  initRangeDepartments = () => {
    const { selectedRange, specificDepartments } = this.getSelectedRange();
    const userDepartmentIdsMap = context.getSetting('userDepartmentIdsMap') || {};
  ...
  renderEmptyTip = () => {
    return (
      <span className="none-search-result d-flex justify-content-center">
        {intl.get('No_departments_available')}
      </span>
    );
  }
  ...
  render() {
    return (
      <div 
        className="dropdown-menu department-editor-list show m-0 p-0" 
      ...
      >
        <div className="search-departments">
          <input
            ...
            type="text"
            autoFocus
            placeholder={intl.get('Find_a_department')}
