...
class MapSelectionEditor extends React.Component {
  ...
  renderSearchResult = () => {
    ...
    return (
      <div className="mobile-dtable-ui-map-selection-editor-search-results">
        {searchResults.map((result, index) => {
          return (
            <div className="mobile-dtable-ui-map-selection-editor-search-result" key={index} onClick={this.onClickSelection.bind(this, result)}>
              <span className="mobile-dtable-ui-map-selection-editor-search-result-title">{result.title || ''}</span>
              <span className="mobile-dtable-ui-map-selection-editor-search-result-address">{result.address || ''}</span>
            </div>
          );
        })}
      </div>
  ...
  renderLabel = () => {
    ...
    if (title) {
      return (
        <div className="mobile-dtable-ui-geolocation-map-editor-label">
          <span className="mobile-dtable-ui-geolocation-map-editor-label-title" title={title}>{title}</span>
          {tagContent && <span className="mobile-dtable-ui-geolocation-map-editor-label-tag">{tagContent}</span>}
          <span className="dtable-font dtable-icon-x" onClick={this.onCloseLabel}></span>
          <span className="mobile-dtable-ui-geolocation-map-editor-label-address-tip">{getLocale('Address')}</span>
          <span className="mobile-dtable-ui-geolocation-map-editor-label-address" title={address}>{address}</span>
          <Button className="mobile-dtable-ui-geolocation-map-editor-label-btn" color="primary" size="sm" onClick={this.onFillIn}>{getLocale('Fill_in')}</Button>
        </div>
      );
    }
    return (
      <div className="mobile-dtable-ui-geolocation-map-editor-label simple">
        <span className="dtable-font dtable-icon-x" onClick={this.onCloseLabel}></span>
        <span className="mobile-dtable-ui-geolocation-map-editor-label-address text-truncate simple" title={address}>{address}</span>
        <Button className="mobile-dtable-ui-geolocation-map-editor-label-btn simple" color="primary" size="sm" onClick={this.onFillIn}>{getLocale('Fill_in')}</Button>
      </div>
    );
  };
    ...
    return (
      <>
        {this.canSelectPosition && (
          <div className="mobile-dtable-ui-geolocation-map-editor-input-container">
            <InputItem
              type="text"
            ...
            />
            <div className="mobile-dtable-ui-geolocation-map-editor-search-btn" onClick={this.onSearch}>
              <i className="dtable-font dtable-icon-search"></i>
            </div>
          </div>
        )}
        <div className="mobile-dtable-ui-geolocation-map-editor-map">
          <div className="w-100 h-100" ref={ref => this.ref = ref} id="mobile-dtable-ui-geolocation-map-container"></div>
  ...
  render() {
    ...
    return (
      <MobileFullScreenPage
        className="mobile-dtable-ui-geolocation-map-selection-editor"
